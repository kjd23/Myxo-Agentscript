...
export default class SlimeMoldModel extends Model {
    ...
    this.stepCount = 0
    this.radius = 1
    // this.meanNumb * this.meanConstant = 450
    this.meanNumb = this.turtleNumber / (2*L) / (2*this.radius) * (this.radius**2 * Math.PI)
    this.meanConstant = 350 / this.meanNumb
    this.initial_waves = 4
    this.turtles.create(this.turtleNumber);
    this.patches.setDefault('pheromone', 0);
    this.u_c = this.meanConstant * this.meanNumb // want 260 * 4 for u_c (calculate meanNumb and change constant for 3.05 * this.meanNumb = 260*4) //  (260*4)**100 = 940, '**' is the XOR operator
    ...
    this.turtles.ask((turtle, turtleIndex) => {
        ...
        // let x = Math.sin(2 * Math.PI * this.initial_waves * turtleIndex / this.turtleNumber)/(2 * Math.PI * this.initial_waves) + (1 * turtleIndex / this.turtleNumber);
        // x = this.world.minX + (this.world.maxX-this.world.minX)*x;
        // let x = Math.sin(turtleIndex * this.meanNumb) * 20 + turtleIndex * this.meanNumb; // original

        // Use randomFloat2 for x position for variation
        let x = util.randomFloat2(-49.9, 49.9);
    ...
    step() {
        ...
        this.turtles.ask((turtle, turtleIndex) => {
            ...
            let theta = 89
            let refrac_rate = 0.56 //0.56

            const mod=util.mod

            const aligned_turtles_ahead = turtles_in_radius
            aligned_turtles_ahead.ask(a => {
                if (mod(Math.abs(turtle.heading - a.heading)) <= theta) {
                    aligned_turtles_ahead.push(a)
                }
            })
            // console.log(aligned_turtles_ahead.length) // check peaks and lows of turtles for u_c parameter
            ...
            let q = 0 // cooperativity
           
            // let lambda = lambda_0 + lambda_c * ((aligned_turtles_ahead.length**q) / (aligned_turtles_ahead.length**q + this.u_c**q))
            let lambda = lambda_0 + lambda_c * ((aligned_turtles_ahead.length**q) / (aligned_turtles_ahead.length**q + this.u_c**q))
